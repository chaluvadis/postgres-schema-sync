...
export const useOperations = (): UseOperationsReturn => {
    ...
    const messageHandler = (event: MessageEvent) => {
      ...
      switch (message.command) {
        case 'operationsLoaded':
          setOperations(message.operations.map((op: any) => ({
            ...op,
          ...
          setOperations(prev => prev.map(op =>
            op.id === message.operationId
              ? {
                  ...
                  steps: message.updates.steps?.map((step: any) => ({
                    ...step,
                    startTime: step.startTime ? new Date(step.startTime) : undefined,
                    endTime: step.endTime ? new Date(step.endTime) : undefined
                  })) || op.steps
  ...
  const startOperation = useCallback(async (operation: Omit<Operation, 'id' | 'status' | 'startTime'>): Promise<string> => {
  ...
  const completedOperations = operations.filter(op => op.status === 'completed' || op.status === 'failed' || op.status === 'cancelled');
