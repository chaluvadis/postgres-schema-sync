    ...
    const messageHandler = (event: MessageEvent) => {
      ...
      switch (message.command) {
        case 'connectionsLoaded':
          setConnections(message.connections.map((conn: any) => ({
            ...conn,
        ...
        case 'connectionStatusChanged':
          setConnections(prev => prev.map(conn =>
            conn.id === message.connectionId
              ? { ...conn, status: message.status, errorMessage: message.errorMessage }
              : conn
          ));
          break;
        case 'connectionTested':
          ...
          if (message.success) {
            ...
            setConnections(prev => prev.map(conn =>
              conn.id === message.connectionId
                ? { ...conn, status: 'error', errorMessage: message.errorMessage }
                : conn
            ));
          }
          break;
