...
export const SchemaBrowser: React.FC = () => {
  ...
  const updateNodeExpansion = (nodes: SchemaNode[], nodeId: string): SchemaNode[] => {
    return nodes.map(node => {
      ...
      if (node.children) {
        return { ...node, children: updateNodeExpansion(node.children, nodeId) };
      }
      return node;
    });
  };
  ...
  const filteredTree = searchTerm
    ? filterTreeBySearch(schemaTree, searchTerm.toLowerCase())
    : schemaTree;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          {loading ? (
            <div style={{ textAlign: 'center', padding: vscodeTheme.spacing.lg }}>
              Loading schema...
            </div>
          ) : (
            <SchemaTree
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{
          ...
          <Button
            onClick={() => vscode.postMessage({ command: 'refreshSchema' })}
            variant="secondary"
            size="sm"
          >
            🔄 Refresh
          ...
          <Button
            onClick={() => vscode.postMessage({ command: 'createMigration' })}
            variant="primary"
            size="sm"
          >
            ⚡ Create Migration
        ...
        <div key={node.id}>
          ...
          >
            {(node.type === 'database' || node.type === 'schema') && (
              <span style={{ marginRight: vscodeTheme.spacing.sm }}>
                {node.expanded ? '📂' : '📁'}
              </span>
            )}
            <span style={{ marginRight: vscodeTheme.spacing.sm }}>
...
const ObjectDetails: React.FC<ObjectDetailsProps> = ({ node }) => {
  if (!node.object) {
    return (
      <div style={{ textAlign: 'center', padding: vscodeTheme.spacing.xl }}>
    ...
    <div>
      ...
      <div style={{ marginBottom: vscodeTheme.spacing.lg }}>
        ...
        <div style={{
          ...
          {obj.modifiedAt && (
            <PropertyItem label="Modified" value={new Date(obj.modifiedAt).toLocaleDateString()} />
          )}
          {obj.sizeInBytes && (
            <PropertyItem label="Size" value={`${(obj.sizeInBytes / 1024).toFixed(2)} KB`} />
          )}
        </div>
      </div>
...
const filterTreeBySearch = (nodes: SchemaNode[], searchTerm: string): SchemaNode[] => {
  return nodes.reduce((filtered: SchemaNode[], node) => {
    ...
    const filteredChildren = node.children
      ? filterTreeBySearch(node.children, searchTerm)
      : [];
