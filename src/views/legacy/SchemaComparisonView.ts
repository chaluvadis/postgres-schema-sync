...
export class SchemaComparisonView {
	...
	async showComparison(comparisonData?: SchemaComparisonData): Promise<void> {
		try {
			...
			vscode.window.showErrorMessage(`Failed to open schema comparison: ${(error as Error).message}`);
		}
	}
	...
	async performDetailedComparison(
		...
		try {
			...
			vscode.window.showErrorMessage(`Detailed schema comparison failed: ${(error as Error).message}`);
			throw error;
		}
	}
        ...
        <body>
            <div class="header">
                ...
                <div class="view-mode-selector">
                    ...
                    <button class="view-mode-btn ${
											hasDetailedData ? "active" : ""
										}" onclick="switchViewMode('detailed')" ${!hasDetailedData ? "disabled" : ""}>Detailed</button>
