...
export const OperationMonitor: React.FC<OperationMonitorProps> = ({
  ...
  const getOperationIcon = (type: string, status: string) => {
    ...
    if (status === 'cancelled') return `${baseIcon}🚫`;
    return baseIcon;
  };

  const getStatusColor = (status: string) => {
    switch (status) {
      case 'running': return vscodeTheme.colors.info;
      case 'completed': return vscodeTheme.colors.success;
      case 'failed': return vscodeTheme.colors.error;
      case 'cancelled': return vscodeTheme.colors.warning;
    ...
    <div style={{
      ...
      {activeOperations.length > 0 && (
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: vscodeTheme.spacing.sm }}>
            {activeOperations.slice(0, 3).map(operation => (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: vscodeTheme.spacing.sm }}>
            ...
            recentOperations.map((operation, index) => (
              <div
                ...
                style={{
                  ...
                  justifyContent: 'space-between',
                  padding: vscodeTheme.spacing.md,
                  borderBottom: index < recentOperations.length - 1
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: vscodeTheme.spacing.sm, flex: 1 }}>
