...
export interface ComplianceViolation {
	...
	objectName?: string;
	schema?: string;
	violationType: "naming" | "structure" | "permission" | "data_quality" | "performance" | "security";
	severity: "low" | "medium" | "high" | "critical";
	description: string;
	actualValue: any;
	expectedValue: any;
...
export class SchemaComparison {
	...
	async compareSchemasDetailed(
		...
		try {
			...
			const sourceTables = await this.schemaOperations.getDatabaseObjects(sourceConnectionId, undefined);
			const targetTables = await this.schemaOperations.getDatabaseObjects(targetConnectionId, undefined);

			const allTableNames = new Set([
				...sourceTables.filter((obj) => obj.type === ObjectType.Table).map((obj) => `${obj.schema}.${obj.name}`),
				...targetTables.filter((obj) => obj.type === ObjectType.Table).map((obj) => `${obj.schema}.${obj.name}`),
			]);
	...
	private async compareTableConstraints(
		...
		try {
			...
			return this.compareConstraintsDetailed(sourceConstraints, targetConstraints);
		} catch (error) {
			Logger.warn("Failed to compare table constraints", "compareTableConstraints", {
				tableName,
				schema,
				error: (error as Error).message,
			});
	...
	private async analyzeViewDependencies(
		...
		const allViews = new Set([
			...sourceTables.filter((obj) => obj.type === ObjectType.View).map((obj) => `${obj.schema}.${obj.name}`),
			...targetTables.filter((obj) => obj.type === ObjectType.View).map((obj) => `${obj.schema}.${obj.name}`),
		]);
		...
		for (const viewIdentifier of allViews) {
			...
			try {
				...
				const viewDependency = this.analyzeViewDependenciesFromMetadata(sourceViewMetadata, targetViewMetadata);
				if (viewDependency) {
					viewDependencies.set(viewIdentifier, viewDependency);
				}
			} catch (error) {
				Logger.warn("Failed to extract view metadata", "analyzeViewDependencies", {
					viewIdentifier,
					sourceConnectionId,
					targetConnectionId,
					error: (error as Error).message,
		...
		for (const columnName of allColumnNames) {
			...
			if (sourceColumn && targetColumn) {
				...
				if (sourceColumn.statistics && targetColumn.statistics) {
					if (
						sourceColumn.statistics.distinctValues !== targetColumn.statistics.distinctValues ||
		...
		for (const indexName of allIndexNames) {
			...
			if (sourceIndex && targetIndex) {
				...
				if (sourceIndex.statistics && targetIndex.statistics) {
					if (
						sourceIndex.statistics.sizeInBytes !== targetIndex.statistics.sizeInBytes ||
	 ...
	 */
	private compareColumnConstraints(sourceConstraints: any[], targetConstraints: any[]): ConstraintDifference[] {
		const differences: ConstraintDifference[] = [];
	 ...
	 */
	private getDifferenceDetails(source: DatabaseObject, target: DatabaseObject, _mode: "strict" | "lenient"): string[] {
	 ...
	 */
	private async getConnectionInfo(connectionId: string): Promise<ConnectionInfo> {
		const connection = await this.schemaOperations.getObjectDetails(connectionId, "connection", "", "");
	...
	private async extractColumnMetadata(
		...
		return await nativeService.extractColumnMetadata(connection, tableName, schema);
	}
	...
	private async extractIndexMetadata(
		...
		const nativeService = PostgreSqlConnectionManager.getInstance();
		return await nativeService.extractIndexMetadata(connection, tableName, schema);
	}
	...
	private async extractConstraintMetadata(
		...
		const nativeService = PostgreSqlConnectionManager.getInstance();
		return await nativeService.extractConstraintMetadata(connection, tableName, schema);
	}
	...
	private async extractViewMetadata(
		...
		const nativeService = PostgreSqlConnectionManager.getInstance();
		return await nativeService.extractViewMetadata(connection, viewName, schema);
	}
