...
export class SchemaComparison {
    ...
    async compareSchemasDetailed(
        ...
        try {
            ...
            for (const tableIdentifier of allTableNames) {
                ...
                try {
                    // Compare columns
                    const columnComparison = await this.compareTableColumns(sourceConnectionId, targetConnectionId, tableName, schema);
                    if (columnComparison.length > 0) {
                        columnComparisons.set(tableIdentifier, columnComparison);
                    }

                    // Compare indexes
                    const indexComparison = await this.compareTableIndexes(sourceConnectionId, targetConnectionId, tableName, schema);
                    if (indexComparison.length > 0) {
                        indexComparisons.set(tableIdentifier, indexComparison);
                    }

                    // Compare constraints
                    const constraintComparison = await this.compareTableConstraints(sourceConnectionId, targetConnectionId, tableName, schema);
            ...
            await this.analyzeViewDependencies(sourceConnectionId, targetConnectionId, sourceTables, targetTables, viewDependencies);
     ...
     */
    private async getConnectionInfo(connectionId: string): Promise<DotNetConnectionInfo> {
    ...
    /**
     * Extract column metadata using DotNet service
     */
    ...
    /**
     * Extract index metadata using DotNet service
     */
    ...
    /**
     * Extract constraint metadata using DotNet service
     */
    ...
    /**
     * Extract view metadata using DotNet service
     */
