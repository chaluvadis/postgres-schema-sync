...
import { DependencyInfo, MetadataManagement, RichMetadataObject } from "./MetadataManagement";
...
export class DependencyAnalysis {
	...
	async buildRichDependencyGraph(connectionId: string, objectIds?: string[]): Promise<RichDependencyGraph> {
		try {
			Logger.info("Building rich dependency graph", "buildRichDependencyGraph", {
				connectionId,
			...
			const objects = objectIds
				? await Promise.all(
						objectIds.map((id) => {
							...
							return this.metadataManagement.getRichMetadataObject(connectionId, objectType, schema, objectName, {
			...
			// Identify clusters
			const clusters = await this.identifyDependencyClusters(objects, richNodes, richEdges);

			// Calculate relationship strengths
			const relationshipStrengths = this.calculateRelationshipStrengths(richEdges);

			// Build lineage chains
			const lineageChains = await this.buildLineageChains(objects, richNodes, richEdges);
			...
			Logger.info("Rich dependency graph completed", "buildRichDependencyGraph", {
	...
	async getObjectLineage(
		...
		try {
			...
			if (direction === "upstream" || direction === "both") {
				await this.traceUpstreamLineage(sourceObject, upstreamObjects, visitedObjects, maxDepth, 0, connectionId);
			}

			if (direction === "downstream" || direction === "both") {
				await this.traceDownstreamLineage(sourceObject, downstreamObjects, visitedObjects, maxDepth, 0, connectionId);
			}

			// Calculate overall lineage depth
			const maxUpstreamDepth = Math.max(...upstreamObjects.map((obj) => obj.distance), 0);
			const maxDownstreamDepth = Math.max(...downstreamObjects.map((obj) => obj.distance), 0);
			...
			const averageConfidence =
				allObjects.length > 0 ? allObjects.reduce((sum, obj) => sum + obj.confidence, 0) / allObjects.length : 1.0;
	...
	private async buildRichDependencyNode(
		...
		const relationshipMetrics = this.calculateRelationshipMetrics(obj, allObjects);
	...
	private calculateRelationshipMetrics(obj: RichMetadataObject, allObjects: RichMetadataObject[]): RelationshipMetrics {
		...
		const relationshipStrength =
			maxPossibleRelationships > 0 ? Math.min(totalRelationships / maxPossibleRelationships, 1) : 0;
		...
		const cohesionLevel =
			dependencyCount > 0 && dependentCount > 0 ? "high" : dependencyCount > 0 || dependentCount > 0 ? "medium" : "low";

		// Calculate complexity score
		const complexityScore = Math.min(dependencyCount * 0.3 + dependentCount * 0.4 + relationshipStrength * 100, 100);
	...
	private countDependents(obj: RichMetadataObject, allObjects: RichMetadataObject[]): number {
		return allObjects.filter((otherObj) => otherObj.dependencies.some((dep) => dep.objectId === obj.id)).length;
	}
	private calculateCentralityScore(obj: RichMetadataObject, allObjects: RichMetadataObject[]): number {
		// Calculate how central this object is in the dependency graph
		const directConnections = obj.dependencies.length + this.countDependents(obj, allObjects);
		const maxConnections = allObjects.length - 1;

		return maxConnections > 0 ? directConnections / maxConnections : 0;
	}
	private calculateInfluenceScore(obj: RichMetadataObject, allObjects: RichMetadataObject[]): number {
	...
	private async buildRichDependencyEdges(
		...
		for (const dep of obj.dependencies) {
			...
			if (targetObj) {
				...
				const relationshipType = this.determineObjectRelationshipType(obj, targetObj, dep);
		...
		for (const otherObj of allObjects) {
			...
			const isDependent = otherObj.dependencies.some((dep) => dep.objectId === obj.id);
			if (isDependent) {
	...
	private areObjectTypesCompatible(type1: string, type2: string): boolean {
		...
		return compatiblePairs.some(([t1, t2]) => (t1 === type1 && t2 === type2) || (t1 === type2 && t2 === type1));
	}
	...
	private assessMaintenanceCost(source: RichMetadataObject, target: RichMetadataObject): "low" | "medium" | "high" {
	...
	private determineObjectRelationshipType(
		...
		const baseRelationship = this.getBaseRelationshipFromDependencyType(dependency);
		if (baseRelationship !== "unknown") {
			return baseRelationship;
		}

		// Then analyze object type combinations for more specific classification
		const objectTypeRelationship = this.getRelationshipFromObjectTypes(source.type, target.type, dependency);
		...
		const semanticRelationship = this.getSemanticRelationship(source, target, dependency);
	...
	private getBaseRelationshipFromDependencyType(
		...
		if (dependency.impactLevel === "critical" && dependency.dependencyType === "hard") {
			return "structural";
		}
	...
	private getRelationshipFromObjectTypes(
		...
		if (normalizedSourceType === "table" && normalizedTargetType === "table") {
			...
			if (description.includes("inherits") || description.includes("parent") || description.includes("child")) {
				return "structural";
			}
		...
		if (normalizedSourceType === "view") {
			...
			if (normalizedTargetType === "view") {
				return "data_flow";
			} // Chained views
			if (normalizedTargetType === "function") {
				return "functional";
			}
			if (normalizedTargetType === "index") {
				return "structural";
			} // Indexed views
		}
		...
		if (normalizedSourceType === "index") {
			...
			if (normalizedTargetType === "column") {
				return "structural";
			}
		}
		...
		if (normalizedSourceType === "column") {
			...
			if (normalizedTargetType === "constraint") {
				return "structural";
			}
			if (normalizedTargetType === "type") {
				return "structural";
		...
		if (normalizedSourceType === "schema") {
			if (
				["table", "view", "function", "index", "constraint", "trigger", "sequence", "type", "domain"].includes(
		...
		if (normalizedSourceType === "role" || normalizedSourceType === "user") {
			if (["table", "view", "function", "schema", "database"].includes(normalizedTargetType)) {
				return "structural";
			}
		}
		...
		if (normalizedSourceType === "extension") {
			if (["function", "type", "table", "schema"].includes(normalizedTargetType)) {
				return "structural";
			}
		}
		...
		if (normalizedSourceType === "foreign table") {
			...
			if (normalizedTargetType === "server") {
				return "structural";
			}
			if (normalizedTargetType === "schema") {
				return "structural";
			}
		}
		...
		if (normalizedSourceType === "language") {
			...
			if (normalizedTargetType === "procedure") {
				return "structural";
			}
		}
	...
	private hasControlFlowPattern(sourceName: string, targetName: string): boolean {
		...
		return (
			controlFlowPrefixes.some((prefix) => sourceLower.startsWith(prefix) || targetLower.startsWith(prefix)) ||
			controlFlowSuffixes.some((suffix) => sourceLower.endsWith(suffix) || targetLower.endsWith(suffix))
		);
	}
	...
	private hasDataFlowPattern(sourceName: string, targetName: string): boolean {
		...
		return (
			dataFlowPrefixes.some((prefix) => sourceLower.startsWith(prefix) || targetLower.startsWith(prefix)) ||
			dataFlowSuffixes.some((suffix) => sourceLower.endsWith(suffix) || targetLower.endsWith(suffix))
		);
	}
	...
	private async identifyDependencyClusters(
		...
		try {
			...
			for (const [schema, objectIds] of schemaGroups.entries()) {
				const clusterNodes = objectIds.map((id) => nodes.get(id)!).filter(Boolean);

				if (clusterNodes.length > 1) {
					// Only create clusters with multiple nodes
					const cohesion = this.calculateClusterCohesion(clusterNodes, edges);
					const coupling = this.calculateClusterCoupling(clusterNodes, edges, nodes);
			...
			Logger.debug("Dependency clusters identified", "identifyDependencyClusters", {
				clusterCount: clusters.length,
				schemaGroups: schemaGroups.size,
			});
		} catch (error) {
			Logger.warn("Failed to identify dependency clusters", "identifyDependencyClusters", {
	...
	private calculateClusterCohesion(nodes: RichDependencyNode[], edges: RichDependencyEdge[]): number {
		// Calculate how well-connected nodes within the cluster are
		const clusterNodeIds = new Set(nodes.map((n) => n.id));
		const internalEdges = edges.filter((edge) => clusterNodeIds.has(edge.source) && clusterNodeIds.has(edge.target));
	...
	private calculateClusterCoupling(
		...
		const maxPossibleExternalEdges = clusterNodes.length * (allNodes.size - clusterNodes.length);
		return maxPossibleExternalEdges > 0 ? externalEdges.length / maxPossibleExternalEdges : 0;
	}
	private calculateClusterStability(nodes: RichDependencyNode[]): number {
		// Calculate average stability of nodes in the cluster
		const totalStability = nodes.reduce((sum, node) => sum + node.stabilityScore, 0);
		return nodes.length > 0 ? totalStability / nodes.length : 0;
	}
	private assessClusterBusinessValue(nodes: RichDependencyNode[]): "low" | "medium" | "high" | "critical" {
		...
		if (hasTables && (hasViews || hasFunctions)) {
			return "high";
		}
		if (hasTables) {
			return "medium";
	...
	private calculateRelationshipStrengths(edges: RichDependencyEdge[]): Map<string, number> {
	...
	private async buildLineageChains(
		...
		try {
			...
			const dataFlowEdges = edges.filter((edge) => edge.relationshipType === "data_flow");
			const functionalEdges = edges.filter((edge) => edge.relationshipType === "functional");
			const structuralEdges = edges.filter((edge) => edge.relationshipType === "structural");
			...
			for (const obj of objects) {
				if (obj.type === "table") {
					...
					if (downstreamObjectIds.length > 1) {
						...
						const complexity = this.assessLineageComplexity(downstreamObjectIds, edges);
						const businessImpact = this.assessLineageBusinessImpact(downstreamObjectIds, objects);
			...
			for (const obj of objects) {
				if (obj.type === "function") {
					...
					if (processChainIds.length > 1) {
						const complexity = this.assessLineageComplexity(processChainIds, edges);
						const businessImpact = this.assessLineageBusinessImpact(processChainIds, objects);
	...
	private assessLineageComplexity(objectIds: string[], edges: RichDependencyEdge[]): "simple" | "moderate" | "complex" {
	...
	private assessLineageBusinessImpact(
		...
		const objects = objectIds.map((id) => allObjects.find((obj) => obj.id === id)).filter(Boolean);

		if (objects.some((obj) => obj?.type === "table")) {
			return "high";
		}
		if (objects.some((obj) => obj?.type === "view")) {
			return "medium";
		}
		return "low";
	}
	private calculateMaxDepthFromNodes(nodes: Map<string, RichDependencyNode>): number {
		let maxDepth = 0;
	...
	private async traceUpstreamLineage(
		...
		for (const dependency of currentObject.dependencies) {
			try {
				...
				const [objectType, schema, objectName] = dependency.objectId.split(":").slice(-3);

				// Get rich metadata for the dependency to understand its type
				const dependencyObject = await this.metadataManagement.getRichMetadataObject(
					connectionId,
					objectType,
					schema,
					objectName,
				...
				if (dependencyObject) {
					...
					const relationshipType = this.determineObjectRelationshipType(currentObject, dependencyObject, dependency);

					// Calculate confidence based on dependency strength and type
					const confidence = this.calculateLineageConfidence(dependency, relationshipType);
					...
					lineageObjects.push({
						...
						transformation: this.extractTransformationInfo(dependency, currentObject, dependencyObject),
						confidence,
					});
				...
				Logger.warn("Failed to trace upstream dependency", "traceUpstreamLineage", {
	...
	private async traceDownstreamLineage(
		...
		try {
			...
			for (const otherObject of allObjects) {
				if (otherObject.id === currentObject.id || visited.has(otherObject.id)) {
					continue;
				}

				// Check if this object depends on our current object
				const isDependent = otherObject.dependencies.some((dep) => dep.objectId === currentObject.id);

				if (isDependent) {
					const dependency = otherObject.dependencies.find((dep) => dep.objectId === currentObject.id)!;

					// Determine relationship type
					const relationshipType = this.determineObjectRelationshipType(otherObject, currentObject, dependency);
					...
					lineageObjects.push({
						...
						relationshipType,
						distance: currentDepth + 1,
						transformation: this.extractTransformationInfo(dependency, otherObject, currentObject),
						confidence,
					});
			...
			Logger.warn("Failed to trace downstream lineage", "traceDownstreamLineage", {
	...
	private findDownstreamObjectsInGraph(
		...
		const downstreamEdges = edges.filter((edge) => edge.source === startObjectId && !visited.has(edge.target));

		for (const edge of downstreamEdges) {
			if (maxDepth > 1) {
				const subChain = this.findDownstreamObjectsInGraph(edge.target, edges, new Set(visited), maxDepth - 1);
	...
	private findProcessLineageInGraph(
		...
		for (const edge of connectedEdges) {
			const connectedObjectId = edge.source === startObjectId ? edge.target : edge.source;

			if (maxDepth > 1) {
				const subChain = this.findProcessLineageInGraph(connectedObjectId, edges, new Set(visited), maxDepth - 1);
				chain.push(...subChain.slice(1)); // Avoid duplicating the current node
			} else {
	...
	private calculateLineageConfidence(dependency: DependencyInfo, relationshipType: string): number {
	...
	private extractTransformationInfo(
		...
		if (description.includes("filter") || description.includes("where") || description.includes("having")) {
			return "Filtering";
		}
	...
	private assessDependencyRisks(objects: RichMetadataObject[], summary: DependencySummary): DependencyRiskAssessment {
		...
		} else if (highFactors > 1) {
			overallRisk = "high";
		} else if (highFactors > 0 || riskFactors.length > 3) {
			overallRisk = "medium";
		}
	...
	private async getAllRichMetadataObjects(connectionId: string): Promise<RichMetadataObject[]> {
		try {
			Logger.info("Retrieving all rich metadata objects for dependency analysis", "getAllRichMetadataObjects", {
				connectionId,
			});

			// Get all database objects from metadata management
			const databaseObjects = await this.metadataManagement.getAllDatabaseObjects(connectionId);

			if (databaseObjects.length === 0) {
				Logger.warn("No database objects found for dependency analysis", "getAllRichMetadataObjects", {
					connectionId,
				});
				return [];
			}
			...
			for (const dbObject of databaseObjects) {
				try {
					...
					const richMetadata = await this.metadataManagement.getRichMetadataObject(
						connectionId,
						dbObject.type,
						dbObject.schema,
						dbObject.name,
			...
			Logger.info("Rich metadata objects retrieved successfully", "getAllRichMetadataObjects", {
				connectionId,
			...
			Logger.error("Failed to get all rich metadata objects", error as Error, "getAllRichMetadataObjects", {
				connectionId,
			});
			return [];
		}
