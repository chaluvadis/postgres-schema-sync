...
export class ConnectionManager {
  ...
  async addConnection(
    connectionInfo: Omit<DatabaseConnection, "id">
  ): Promise<void> {
    try {
  ...
  async updateConnection(
    ...
    try {
      ...
      Logger.error(
        `Failed to update connection: ${(error as Error).message}`,
        error as Error,
        "updateConnection"
      );
      throw error;
    ...
    try {
      ...
      if (!connection) {
        Logger.error(
          ...
          new Error(`Connection ${id} not found`),
          "testConnection"
        );
        return false;
      }
      ...
      if (!this.dotNetService) {
        Logger.error(
          ...
          new Error("DotNet service is null"),
          "testConnection"
        );
        return false;
      }
      ...
      if (!password) {
        Logger.error(
          `Password not found for connection: ${id}`,
          new Error("Password not available"),
          "testConnection"
        );
        throw new Error(
      ...
      if (!this.validateConnectionInfo(connection)) {
        ...
        throw new Error(
          "Connection parameters are invalid. Please check host, port, and database name."
        );
      }
      ...
      if (connection.port === 5432) {
        ...
        if (!securityValidation.allowed) {
          Logger.warn(
            `Security validation failed for connection ${id}`,
            "testConnection"
          );
          if (!securityValidation.requiresSSL) {
            throw new Error(
              `Security policy violation: ${securityValidation.reason}`
            );
          }
        }
        ...
        try {
          ...
          if (!certValidation.valid) {
            Logger.warn(
              `Certificate validation failed for ${connection.host}`,
              "testConnection"
            );
            vscode.window
              .showWarningMessage(
      ...
      Logger.info(
        `Connection test ${success ? "successful" : "failed"}: ${id}`,
        "testConnection"
      );
      return success;
    } catch (error) {
      Logger.error(
        `Connection test error: ${(error as Error).message}`,
        error as Error,
        "testConnection"
      );
      ...
      if (
        ...
        vscode.window.showErrorMessage(
          `Connection failed: Authentication error. Please check username and password.`
        );
      } else if (
        ...
        vscode.window.showErrorMessage(
          `Connection failed: Network error. Please check host and port.`
        );
      } else {
        vscode.window.showErrorMessage(
  ...
  async testConnectionData(
    connectionData: Omit<DatabaseConnection, "id"> & { password: string }
  ): Promise<boolean> {
    try {
      ...
      Logger.info(
        `Connection test ${success ? "successful" : "failed"}: ${connectionData.name
        }`
      );
      return success;
    } catch (error) {
  ...
  private async loadConnections(): Promise<void> {
    try {
      const connectionsData = this.context.globalState.get<string>(
  ...
  async dispose(): Promise<void> {
    try {
      ...
      for (const [connectionId, activeConnectionIds] of this
        ...
        for (const activeConnectionId of activeConnectionIds) {
          try {
            ...
            Logger.warn(
              `Error closing connection ${activeConnectionId}`,
              "dispose"
            );
          }
        }
