...
export class EnhancedStatusBarProvider {
	...
	private constructor(connectionManager: ConnectionManager, notificationManager: NotificationManager) {
	...
	static getInstance(
		...
		if (!EnhancedStatusBarProvider.instance) {
			EnhancedStatusBarProvider.instance = new EnhancedStatusBarProvider(connectionManager, notificationManager);
		}
	...
	private updateOperationIndicators(): void {
		...
		const localOperations = Array.from(this.operationIndicators.values()).filter(
	...
	cancelOperation(id: string): boolean {
		...
		try {
			...
			Logger.info("Operation cancelled", "cancelOperation", {
				operationId: id,
			});
			return true;
		} catch (error) {
	...
	getActiveOperations(): OperationIndicator[] {
		return Array.from(this.operationIndicators.values()).filter(
			(op) => op.status === "running" || op.status === "pending",
		);
	}
	updateOperation(
	...
	completeOperation(id: string): void {
		...
		Logger.debug("Operation completed", "completeOperation", {
			operationId: id,
		});
	}
