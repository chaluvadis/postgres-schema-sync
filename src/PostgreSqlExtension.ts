...
export class PostgreSqlExtension {
	...
	async testConnection(connection: any): Promise<void> {
		const context = ErrorHandler.createEnhancedContext("TestConnection", {
					...
					try {
						...
						try {
							...
							ErrorHandler.handleError(
								error,
								ErrorHandler.createContext("ConnectionTestExecution", {
									connectionId: connection.id,
									connectionName: connection.name,
								}),
							);
						...
						progress.report({
							increment: 50,
							message: "Running connectivity tests...",
						});
						...
						if (success) {
							...
							vscode.window.showErrorMessage(errorMsg, "Edit Connection", "View Logs", "Get Help").then((selection) => {
			...
			await vscode.window.withProgress(
				...
				async (progress) => {
					...
					try {
						...
						vscode.window
							.showErrorMessage(`Migration "${migration.name}" failed: ${(error as Error).message}`, "View Logs")
							.then((selection) => {
								if (selection === "View Logs") {
									Logger.showOutputChannel();
								}
	...
	async dispose(): Promise<void> {
		const disposeContext = ErrorHandler.createEnhancedContext("ExtensionDisposal", {
			graceful: true,
		...
		try {
			...
			ErrorHandler.handleErrorWithSeverity(error as Error, disposeContext, ErrorSeverity.HIGH);
