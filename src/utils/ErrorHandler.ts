...
export class ErrorHandler {
				...
				actions.push(
					{
						...
						action: async () => {
							await vscode.commands.executeCommand("postgresql.validateMigration");
						},
						primary: true,
					},
					{
						...
						action: async () => {
							await vscode.commands.executeCommand("postgresql.showLogs");
						},
					},
					{
						id: "dry_run",
				...
				actions.push(
					{
						...
						action: async () => {
							await vscode.commands.executeCommand("postgresql.refreshExplorer");
						},
						primary: true,
					},
					{
				...
				actions.push(
					{
						...
						action: async () => {
							await vscode.commands.executeCommand("postgresql.showDashboard");
						},
						primary: true,
					},
					{
						...
						action: async () => {
							await vscode.commands.executeCommand("postgresql.validateData");
						},
					},
				);
				break;
				...
				actions.push(
					{
						...
						action: async () => {
							await vscode.commands.executeCommand("postgresql.showQueryAnalytics");
						},
						primary: true,
					},
					{
						...
						action: async () => {
							await vscode.window.showInformationMessage("Performance optimization suggestions coming soon");
						},
					},
				);
				break;
			...
			default:
				actions.push(
					...
					},
					{
						...
						action: async () => {
							await vscode.commands.executeCommand("postgresql.showHelp");
						},
					},
				);
		}
		...
		switch (severity) {
			case ErrorSeverity.CRITICAL:
				if (error instanceof Error) {
					...
					Logger.critical(errorMessage, context.operation, {
						...context.contextData,
						originalError: String(error),
					});
				}
	...
	private static shouldShowUserNotification(errorMessage: string): boolean {
		...
		return userNotificationKeywords.some((keyword) => errorMessage.toLowerCase().includes(keyword));
	}
	private static showUserErrorNotification(errorMessage: string, context: ErrorContext): void {
		...
		vscode.window
			.showErrorMessage(`PostgreSQL Schema Sync: ${errorMessage}`, ...(allActions as [string, ...string[]]))
	...
	private static showErrorDetails(errorDetails: ErrorDetails): void {
		const panel = vscode.window.createWebviewPanel("errorDetails", "Error Details", vscode.ViewColumn.One, {
			enableScripts: true,
		});
