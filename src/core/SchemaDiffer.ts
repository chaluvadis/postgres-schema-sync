...
export class SchemaDiffer {
	...
	private createDifference(
		...
		switch (type) {
			...
			case "alter":
				if (!targetObj) {
					throw new Error("Target object required for alter");
				}
				return {
					...baseDiff,
	...
	private parseTableColumns(definition: string): any[] {
		try {
			...
			Logger.warn("AST parsing failed, falling back to regex parsing", "SchemaDiffer.parseTableColumns", {
				error: String(error),
			});
		}
	...
	private parseTableColumnsRegex(definition: string): any[] {
		...
		try {
			...
			Logger.debug(
				"AST parsing failed for table columns, using regex fallback",
				"SchemaDiffer.parseTableColumnsRegex",
				{
	...
	private parseTableConstraints(definition: string): any[] {
		...
		try {
			...
			Logger.debug(
				"AST parsing failed for table constraints, using regex fallback",
				"SchemaDiffer.parseTableConstraints",
				{
					error: String(astError),
				},
			);
		...
		if (pkMatch) {
			constraints.push({
				type: "PRIMARY KEY",
				columns: pkMatch[1].split(",").map((s) => s.trim()),
			});
		}
	...
	private extractFunctionSignature(definition: string): string {
		try {
			...
			Logger.debug(
				"AST parsing failed for function signature, using regex fallback",
				"SchemaDiffer.extractFunctionSignature",
				{
					error: String(astError),
				},
			);
	...
	private parseSequenceParams(definition: string): any {
		try {
			...
			if (ast && Array.isArray(ast) && ast.length > 0) {
				...
				if (createSequenceAst.type === "create" && createSequenceAst.keyword === "sequence") {
					const params: any = {};
					if (createSequenceAst.start) {
						params.start = parseInt(createSequenceAst.start);
					}
					if (createSequenceAst.increment) {
						params.increment = parseInt(createSequenceAst.increment);
					}
					if (createSequenceAst.minvalue) {
						params.minvalue = parseInt(createSequenceAst.minvalue);
					}
					if (createSequenceAst.maxvalue) {
						params.maxvalue = parseInt(createSequenceAst.maxvalue);
					}
					return params;
				}
			}
		...
		if (startMatch) {
			params.start = parseInt(startMatch[1]);
		}

		const incrementMatch = definition.match(/INCREMENT\s+(?:BY\s+)?(\d+)/i);
		if (incrementMatch) {
			params.increment = parseInt(incrementMatch[1]);
		}
	...
	private generateAlterTableSql(from: SchemaObject, to: SchemaObject): string {
		...
		for (const fromCol of fromColumns) {
			...
			if (!toCol) {
				...
				const isReferenced = fromConstraints.some(
					(con) => con.type === "FOREIGN KEY" && con.columns.includes(fromCol.name),
				);
	...
	private generateConstraintChanges(
		from: SchemaObject,
		to: SchemaObject,
		fromConstraints: any[],
		toConstraints: any[],
		...
		for (const toConstraint of toConstraints) {
			const exists = fromConstraints.some(
				(fc) => fc.type === toConstraint.type && JSON.stringify(fc.columns) === JSON.stringify(toConstraint.columns),
			);
			if (!exists) {
				switch (toConstraint.type) {
					...
					case "FOREIGN KEY":
						statements.push(
							`ALTER TABLE ${tableName} ADD FOREIGN KEY (${toConstraint.columns.join(", ")}) REFERENCES ${toConstraint.refTable} (${toConstraint.refColumns.join(", ")});`,
						);
						break;
					case "UNIQUE":
	...
	private extractReferencedTables(sql: string): string[] {
		...
		try {
						...
						if (stmt.expr && stmt.expr.type === "alter") {
							for (const action of stmt.expr.actions || []) {
								if (
									...
									action.constraint_type === "foreign key"
								) {
			...
			Logger.debug(
				"AST parsing failed for referenced tables, using regex fallback",
				"SchemaDiffer.extractReferencedTables",
				{
					error: String(astError),
				},
			);
