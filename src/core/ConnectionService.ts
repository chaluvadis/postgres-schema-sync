...
import { ConnectionInfo, PostgreSqlConnectionManager } from "./PostgreSqlConnectionManager";
...
export class ConnectionService {
	...
	async toDotNetConnection(connectionId: string): Promise<ConnectionInfo | null> {
		try {
			...
			Logger.error("Failed to convert to DotNet connection", error as Error, "ConnectionService.toDotNetConnection", {
				connectionId,
			});
			return null;
		}
