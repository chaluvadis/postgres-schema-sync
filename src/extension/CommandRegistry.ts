    ...
    registerCommand(command: string, handler: (...args: any[]) => any, description?: string): void {
        try {
            const disposable = vscode.commands.registerCommand(command, async (...args: any[]) => {
                try {
                    Logger.debug('Executing command', 'CommandRegistry', { command, argCount: args.length });
                    await handler(...args);
            ...
            this.registeredCommands.add(command);
            Logger.debug('Command registered successfully', 'CommandRegistry', { command, description });
        } catch (error) {
