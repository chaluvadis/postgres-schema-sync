...
export class SQLFileHandlers {
	...
	async executeCurrentSQLFile(): Promise<void> {
		...
		if (document.languageId !== "sql" && document.languageId !== "postgresql") {
			Logger.warn("Current file is not a SQL file", "SQLFileHandlers", {
		...
		if (sqlContent.length > maxSqlLength) {
			Logger.warn("SQL file too large", "SQLFileHandlers", {
		...
		for (const pattern of dangerousPatterns) {
			if (pattern.test(sqlContent)) {
				Logger.warn("Potentially dangerous SQL detected", "SQLFileHandlers", {
	...
	async formatCurrentSQLFile(): Promise<void> {
		...
		try {
			...
			const fullRange = new vscode.Range(document.positionAt(0), document.positionAt(sqlContent.length));
			edit.replace(document.uri, fullRange, formattedSQL);

			const success = await vscode.workspace.applyEdit(edit);
			if (success) {
				Logger.info("SQL file formatted successfully", "SQLFileHandlers", {
					fileName: document.fileName,
				});
	...
	private async selectConnection(): Promise<DatabaseConnection | undefined> {
		...
		if (detectedConnectionId) {
			...
			if (detectedConnection) {
				...
				Logger.info("Using detected connection", "SQLFileHandlers", {
					connectionId: detectedConnectionId,
				});
