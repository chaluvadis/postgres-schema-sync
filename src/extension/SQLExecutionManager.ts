		...
		await vscode.window.withProgress(
			...
			async (progress, token) => {
				...
				for (let i = 0; i < statements.length; i++) {
					...
					try {
						const result = await queryExecutionService.executeQuery(connectionId, trimmedStatement, {
							timeout: 30000,
							maxRows: 1000,
						});
...
export async function formatSQL(sqlContent: string): Promise<string> {
	try {
		...
		formatted = formatted.replace(
			/\s*(SELECT|FROM|WHERE|GROUP BY|ORDER BY|HAVING|INSERT|UPDATE|DELETE|CREATE|DROP|ALTER)\s+/gi,
			"\n$1 ",
		);
...
function showExecutionResults(successCount: number, errorCount: number, totalCount: number): void {
	...
	const avgDuration =
		queryExecutionState.executionResults.length > 0
			? queryExecutionState.executionResults.reduce((sum, result) => sum + result.duration, 0) /
				queryExecutionState.executionResults.length
			: 0;
		...
		if (successfulResults.length > 0) {
			...
			successfulResults
				...
				.forEach((result, index) => {
					...
					details.push(
						`${index + 1}. ${result.duration}ms ${performanceIndicator} - ${result.statement.substring(0, 60)}${result.statement.length > 60 ? "..." : ""}`,
					);
				});
		}
...
function generatePerformanceRecommendations(
	...
	if (slowQueries.length > 0) {
		recommendations.push(
			`${slowQueries.length} queries are significantly slower than average - consider adding indexes`,
		);
	}
...
function getAverageDurationRating(avgDuration: number): {
	rating: string;
	description: string;
	severity: number;
} {
	if (avgDuration < 50) {
		return {
			rating: "Excellent",
			description: "Very fast individual query performance",
			severity: 1,
		};
	} else if (avgDuration < 150) {
		return {
			rating: "Good",
			description: "Fast individual query performance",
			severity: 2,
		};
	} else if (avgDuration < 500) {
		return {
			rating: "Moderate",
			description: "Acceptable individual query performance",
			severity: 3,
		};
	} else if (avgDuration < 1000) {
		return {
			rating: "Slow",
			description: "Slow individual query performance",
			severity: 4,
		};
	} else {
		return {
			rating: "Very Slow",
			description: "Very slow individual query performance",
			severity: 5,
		};
	}
}

function getTotalDurationRating(totalDuration: number): {
	rating: string;
	description: string;
	severity: number;
} {
	if (totalDuration < 100) {
		return {
			rating: "Excellent",
			description: "Very fast overall execution",
			severity: 1,
		};
	} else if (totalDuration < 500) {
		return {
			rating: "Good",
			description: "Fast overall execution",
			severity: 2,
		};
	} else if (totalDuration < 2000) {
		return {
			rating: "Moderate",
			description: "Acceptable overall execution time",
			severity: 3,
		};
	} else if (totalDuration < 5000) {
		return {
			rating: "Slow",
			description: "Slow overall execution, consider optimization",
			severity: 4,
		};
	} else {
		return {
			rating: "Very Slow",
			description: "Very slow overall execution, optimization required",
			severity: 5,
		};
	}
}
