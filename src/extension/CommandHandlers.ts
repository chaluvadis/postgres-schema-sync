        ...
        try {
            ...
            if (this.components.connectionManager) {
                ...
                if (confirm === 'Remove') {
                    ...
                    Logger.info('Connection removed successfully', 'CommandHandlers', { connectionId: connection.id });
    ...
    async handleBrowseSchema(connectionId?: string, schemaName?: string): Promise<void> {
        try {
            ...
            if (this.components.schemaBrowserView) {
                ...
                Logger.info('Schema browser opened successfully', 'CommandHandlers', { connectionId, schemaName });
            } else {
    ...
    async handleViewObjectDetails(databaseObject?: any): Promise<void> {
        try {
            ...
            if (this.components.schemaBrowserView) {
                ...
                Logger.info('Object details displayed with real-time metadata', 'CommandHandlers', { object: databaseObject.name });
            } else {
                vscode.window.showErrorMessage('Schema browser not available');
            }
        } catch (error) {
