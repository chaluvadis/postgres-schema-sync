...
export class RealtimeMonitoringManager {
  ...
  private async testConnectionQuietly(connectionId: string): Promise<boolean> {
    try {
      ...
      const connection =
        this.components?.connectionManager?.getConnection(connectionId);
      if (!connection) {
        Logger.warn(
          "Connection not found for quiet test",
      ...
      if (!password) {
        Logger.warn(
          "Password not available for connection test",
          "testConnectionQuietly",
          { connectionId }
        );
        return false;
  ...
  private async testConnectionWithDotNet(
    dotNetConnection: any
  ): Promise<boolean> {
    try {
