...
export class EventHandlerManager {
  ...
  private registerTextEditorEvents(): void {
    ...
    this.context.subscriptions.push(
      vscode.window.onDidChangeActiveTextEditor((editor) => {
        ...
        const isSQLFile =
          document.languageId === "sql" || document.languageId === "postgresql";
        ...
        if (
          ...
          vscode.commands.executeCommand(
            "setContext",
            "postgresql.sqlFileActive",
            true
          );
          vscode.commands.executeCommand(
            "setContext",
            "postgresql.sqlFilePath",
            document.fileName
          );
    ...
    this.context.subscriptions.push(
      vscode.window.onDidChangeWindowState((state) => {
        if (state.focused && realtimeState.activeSQLFile) {
          ...
          Logger.debug(
            "Window focused, refreshing real-time state",
    ...
    try {
      ...
      for (const part of pathParts) {
        const matchingConnection = connections.find(
          (conn) => part.includes(conn.database) || part.includes(conn.name)
        );
        if (matchingConnection) {
          vscode.commands.executeCommand(
            "setContext",
            "postgresql.detectedConnection",
            matchingConnection.id
          );
          Logger.debug(
            "Auto-detected connection for SQL file",
            "detectConnectionForSQLFile",
            {
              fileName,
      ...
      for (const connection of connections) {
        if (
          content.includes(connection.host) ||
          ...
          Logger.debug(
            "Connection detected in SQL content",
            "detectConnectionForSQLFile",
            {
              fileName,
  ...
  private initializePersistentStatusBar(): void {
    if (!realtimeState.statusBarItem) {
      realtimeState.statusBarItem = vscode.window.createStatusBarItem(
  ...
  private updatePersistentStatusBar(document: vscode.TextDocument): void {
    ...
    realtimeState.statusBarItem.tooltip = `SQL File: ${
      document.fileName
    }\nLanguage: ${
      document.languageId
  ...
  private getCurrentConnectionInfo(): string {
    const detectedConnectionId = vscode.workspace
      .getConfiguration()
      .get<string>("postgresql.detectedConnection");
    ...
    watcher.onDidChange((uri) => {
      ...
      vscode.window
        .showInformationMessage(
          `SQL file "${document.fileName
            .split(/[/\\]/)
            .pop()}" was modified externally`,
          "Refresh",
          "Ignore"
        )
        .then((selection) => {
  ...
  private refreshIntelliSenseForFile(document: vscode.TextDocument): void {
    try {
      const content = document.getText();
      const connectionId = vscode.workspace
        .getConfiguration()
        .get<string>("postgresql.detectedConnection");
      ...
      if (connectionId && this.components?.queryEditorView) {
        ...
        Logger.debug(
          "Refreshing IntelliSense for SQL file",
          "refreshIntelliSenseForFile",
          {
            fileName: document.fileName,
            connectionId,
          }
  ...
  private startSchemaMonitoring(document: vscode.TextDocument): void {
    ...
    let connectionId = vscode.workspace
      .getConfiguration()
      .get<string>("postgresql.detectedConnection");

    // If no configured connection, try to detect from document content
    if (!connectionId) {
      const detectedConnectionId = this.detectConnectionFromDocument(
        fileContent,
        fileName
      );
    ...
    if (!connectionId || !this.components?.schemaManager) {
      Logger.debug(
        "Schema monitoring skipped - no valid connection",
    ...
    Logger.info(
      "Starting schema monitoring for SQL file",
      "startSchemaMonitoring",
      {
        fileName,
        connectionId,
  ...
  private async checkSchemaChanges(
    connectionId: string,
    ...
    try {
      ...
      if (!this.components?.schemaManager) {
        Logger.debug(
          "Schema manager not available for change detection",
          "checkSchemaChanges"
        );
        return;
      }
      ...
      if (this.hasSchemaChanged(previousSchema, currentSchema)) {
        ...
        const fileName =
          document?.fileName.split(/[/\\]/).pop() || "current file";
        vscode.window
          .showInformationMessage(
  ...
  private detectConnectionFromDocument(
    content: string,
    fileName: string
  ): string | null {
    try {
      ...
      for (const connection of connections) {
        ...
        if (
          ...
          Logger.debug(
            "Connection detected in document content",
            "detectConnectionFromDocument",
            {
              fileName,
              detectedConnection: connection.name,
      ...
      for (const part of pathParts) {
        ...
        if (matchingConnection) {
          Logger.debug(
            "Connection detected in file path",
            "detectConnectionFromDocument",
            {
              fileName,
              detectedConnection: matchingConnection.name,
      ...
      Logger.debug(
        "No connection detected for document",
        "detectConnectionFromDocument",
        { fileName }
      );
      return null;
      ...
      Logger.error(
        "Error detecting connection from document",
        error as Error,
        "detectConnectionFromDocument",
        {
          fileName,
  ...
  private async getCurrentSchemaState(connectionId: string): Promise<string> {
    try {
      ...
      const objects = await this.components.schemaManager.getDatabaseObjects(
        connectionId
      );
      ...
      Logger.error(
        "Error getting current schema state",
        error as Error,
    ...
    try {
      ...
      if (cachedEntry) {
        ...
        if (cacheAge < maxCacheAge) {
          Logger.debug(
            "Using in-memory cached schema state",
            "getCachedSchemaState",
            {
              connectionId,
          ...
          Logger.debug(
            "Cached schema state expired, removed from cache",
            "getCachedSchemaState",
            {
              connectionId,
              cacheAge: Math.round(cacheAge / 1000),
      ...
      if (persistentCache) {
        ...
        if (cacheAge < maxPersistentAge) {
          ...
          Logger.debug(
            "Using persistent cached schema state",
            "getCachedSchemaState",
            {
              connectionId,
          ...
          Logger.debug(
            "Persistent cached schema state expired",
            "getCachedSchemaState",
            {
              connectionId,
              cacheAge: Math.round(cacheAge / 1000 / 60 / 60),
      ...
      Logger.debug(
        "No valid cached schema state found",
        "getCachedSchemaState",
        { connectionId }
      );
      return "";
      ...
      Logger.error(
        "Error retrieving cached schema state",
        error as Error,
        "getCachedSchemaState",
        {
          connectionId,
   ...
   * Update cached schema state
   */
  private updateCachedSchemaState(
    connectionId: string,
    schemaState: string,
    objectCount: number = 0,
    cachedBy: string = "system"
  ): void {
    try {
      ...
      Logger.debug(
        "Schema state cached successfully",
        "updateCachedSchemaState",
        {
          connectionId,
      ...
      Logger.error(
        "Error updating cached schema state",
        error as Error,
        "updateCachedSchemaState",
        {
          connectionId,
   ...
   */
  private savePersistentSchemaCache(
    connectionId: string,
    ...
    try {
      ...
      Logger.debug(
        "Schema cache saved to persistent storage",
        "savePersistentSchemaCache",
        {
          connectionId,
          cacheKey,
        }
      ...
      Logger.error(
        "Error saving persistent schema cache",
        error as Error,
        "savePersistentSchemaCache",
        {
          connectionId,
   ...
   */
  private loadPersistentSchemaCache(
    connectionId: string
  ): SchemaCacheEntry | null {
    try {
      const cacheKey = `postgresql.schemaCache.${connectionId}`;
      const cachedData = this.context.workspaceState.get(cacheKey) as
        | PersistentSchemaCacheData
        | undefined;
      ...
      if (cachedData) {
        ...
        if (cachedData.schemaFingerprint && cachedData.timestamp) {
          ...
          Logger.debug(
            "Schema cache loaded from persistent storage",
            "loadPersistentSchemaCache",
            {
              connectionId,
              cacheAge: Math.round(
                (Date.now() - cacheEntry.timestamp) / 1000 / 60
              ),
            }
          );
      ...
      Logger.error(
        "Error loading persistent schema cache",
        error as Error,
        "loadPersistentSchemaCache",
        {
          connectionId,
  ...
  private clearPersistentSchemaCache(connectionId: string): void {
    try {
      ...
      Logger.debug(
        "Persistent schema cache cleared",
        "clearPersistentSchemaCache",
        {
          connectionId,
          cacheKey,
        }
      ...
      Logger.error(
        "Error clearing persistent schema cache",
        error as Error,
        "clearPersistentSchemaCache",
        {
          connectionId,
  ...
  clearAllSchemaCaches(): void {
    try {
      ...
      Logger.error(
        "Error clearing all schema caches",
        error as Error,
        "clearAllSchemaCaches"
      );
      vscode.window.showErrorMessage(
        `Failed to clear schema caches: ${(error as Error).message}`
      );
    }
  }
  ...
  getSchemaCacheStats(): {
    ...
    try {
      ...
      Logger.error(
        "Error getting schema cache statistics",
        error as Error,
        "getSchemaCacheStats"
      );
      return {
  ...
  private async testConnectionQuietly(connectionId: string): Promise<boolean> {
    ...
    try {
      ...
      if (!this.components?.connectionManager) {
        Logger.warn(
          "Connection manager not available for quiet testing",
          "testConnectionQuietly",
          {
            connectionId,
          }
        );
      ...
      if (!connection) {
        Logger.warn(
          "Connection not found for quiet testing",
          "testConnectionQuietly",
          {
            connectionId,
          }
      ...
      const isConnected = await this.performLightweightConnectionTest(
        connection
      );
   ...
   */
  private async performLightweightConnectionTest(
    connection: any
  ): Promise<boolean> {
    ...
    try {
      Logger.debug(
        "Starting real-time lightweight connection test",
        "performLightweightConnectionTest",
        {
          connectionId: connection.id,
      ...
      if (!this.components?.connectionManager) {
        Logger.warn(
          "Connection manager not available for real-time testing",
          "performLightweightConnectionTest",
          {
            connectionId: connection.id,
          }
      ...
      const connectionInfo =
        await this.components.connectionManager.getConnection(connection.id);
      ...
      const password =
        await this.components.connectionManager.getConnectionPassword(
          connection.id
        );

      if (!password) {
        Logger.warn(
          "Connection password not available",
          "performLightweightConnectionTest",
          {
            connectionId: connection.id,
          }
      ...
      const dotNetConnection = {
        ...
        password: password,
        createdDate:
          connectionInfo.lastConnected?.toISOString() ||
      ...
      if (!this.validateConnectionObject(dotNetConnection)) {
        Logger.error(
          "Invalid connection object for testing",
          "performLightweightConnectionTest",
          {
            connectionId: connection.id,
            hasRequiredFields: !!(
      ...
      const testPromise = dotNetService
        .testConnection(dotNetConnection)
        .then((result) => {
        ...
        .catch((error) => {
          const testDuration = Date.now() - testStartTime;
          Logger.debug(
            "Real-time connection test failed",
            "performLightweightConnectionTest",
            {
              connectionId: connection.id,
              connectionName: connection.name,
      ...
      Logger.debug(
        "Lightweight connection test completed",
        "performLightweightConnectionTest",
        {
          connectionId: connection.id,
          result,
      ...
      Logger.error(
        "Error in real-time lightweight connection test",
        error as Error,
        "performLightweightConnectionTest",
        {
          connectionId: connection.id,
  ...
  private validateConnectionObject(connection: any): boolean {
    try {
      ...
      if (missingFields.length > 0) {
        Logger.warn(
          "Connection object missing required fields",
      ...
      Logger.debug(
        "Connection object validation passed",
        "validateConnectionObject",
        {
          connectionId: connection.id,
          host: connection.host,
      ...
      Logger.error(
        "Error validating connection object",
        error as Error,
        "validateConnectionObject",
        {
          connectionId: connection.id,
  ...
  private setupWorkspaceSQLWatchers(): void {
    ...
    watcher.onDidDelete((uri) => {
      Logger.info(
        "SQL file removed from workspace",
        "setupWorkspaceSQLWatchers",
        { filePath: uri.fsPath }
      );
        ...
        case "averageResponseTime":
          if (responseTime) {
            ...
            performanceMetrics.averageResponseTime =
              (current * count + responseTime) / (count + 1);
          }
          break;
      }
  ...
  private logPerformanceSummary(): void {
    ...
    const avgResponseTime =
      performanceMetrics.averageResponseTime > 0
        ? Math.round(performanceMetrics.averageResponseTime)
        : 0;
  ...
  private updateTreeViewTitle(): void {
    try {
      const connectionCount =
        this.components?.connectionManager?.getConnections().length || 0;
  ...
  private trackTreeViewExpansion(element: any, expanded: boolean): void {
    try {
      ...
      if (expanded) {
        Logger.debug(
          "Element expanded for real-time tracking",
          "trackTreeViewExpansion",
          {
            elementKey,
            expanded,
          }
  ...
  private cleanupRealtimeMonitoring(): void {
    Logger.info(
      "Cleaning up real-time monitoring",
  ...
  registerCacheManagementCommands(): void {
    ...
    Logger.info(
      "Cache management commands registered",
      "registerCacheManagementCommands"
    );
  }
...
Generated at: ${new Date().toLocaleString()}
  ...
  cleanupExpiredCaches(): void {
    try {
      ...
      const initialPersistentKeys = this.context.workspaceState
        .keys()
        .filter((key) => key.startsWith("postgresql.schemaCache.")).length;
      ...
      for (const key of allKeys) {
        if (key.startsWith("postgresql.schemaCache.")) {
          const cachedData = this.context.workspaceState.get(key) as
            | PersistentSchemaCacheData
            | undefined;
