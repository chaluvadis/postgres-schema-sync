...
import { DetailedSchemaComparisonResult, SchemaDifference } from "@/managers/schema/SchemaComparison";
...
export class ReportingService {
	...
	async recordComparison(
		...
		try {
			...
			const entry: ComparisonHistoryEntry = {
				...
				createdAt:
					result.createdAt instanceof Date ? result.createdAt.toISOString() : new Date(result.createdAt).toISOString(),
			...
			await this.context.globalState.update(`${DETAIL_PREFIX}${entry.id}`, sanitizedDifferences);
	...
	async clearComparisonHistory(): Promise<void> {
		...
		await Promise.all(
			history.map((entry) => this.context.globalState.update(`${DETAIL_PREFIX}${entry.id}`, undefined)),
		);
	...
	private sanitiseDifference(diff: SchemaDifference): SanitizedDifference {
		return {
			...
			schema: diff.schema,
			differenceDetails: (diff.differenceDetails || []).slice(0, 5).map((detail) => this.sanitiseDetailText(detail)),
		};
	}

	private sanitizeDifferences(differences: SchemaDifference[]): SanitizedDifference[] {
		const sanitised = differences.slice(0, MAX_DETAILS_PER_ENTRY).map((diff) => this.sanitiseDifference(diff));
	...
	private buildHighlights(differences: SanitizedDifference[], summary: Record<string, number>): string[] {
