...
export class DataImportService {
		...
		if (nonNullValues.length > 0) {
			...
			if (allNumbers) {
				...
				if (allDates) {
					...
					const allBooleans = nonNullValues.every((val) => booleanValues.includes(val.toLowerCase()));
					if (allBooleans) {
	...
	private async performDataQualityAnalysis(
		...
		columns.forEach((column) => {
			...
			const completenessScore =
				previewData.length > 0 ? ((previewData.length - nullCount) / previewData.length) * 100 : 0;
			...
			if (column.type === "string" && columnValues.length > 1) {
				...
				const mostCommonFormat = formats.reduce(
					(acc, format) => {
		...
		try {
			...
			for (let i = 0; i < rows.length; i += batchSize) {
				...
				try {
					...
					const sql = this.generateBatchInsertSQL(job.targetTable!, job.targetSchema!, columnMapping, transformedBatch);
	...
	async createImportTemplate(templateData: Omit<any, "id" | "createdAt" | "updatedAt" | "usageCount">): Promise<any> {
		try {
			const template: any = {
				...templateData,
				id: this.generateId(),
	...
	getImportStatistics(): {
		...
		const formatCount = completedJobs.reduce(
			(acc, job) => {
