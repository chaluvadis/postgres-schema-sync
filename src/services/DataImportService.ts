...
export class DataImportService {
  ...
  private loadImportData(): void {
    try {
      ...
      const templatesData = this.context.globalState.get<string>(
      ...
      // Load import history
      const historyData = this.context.globalState.get<string>(
        "postgresql.imports.history",
        "[]"
      );
  ...
  async analyzeImportFile(jobId: string): Promise<{
    ...
    try {
      ...
      const recommendedMappings =
        this.generateRecommendedMappings(detectedColumns);
      ...
      if (job.options.validateData !== false) {
        const qualityResult = await this.performDataQualityAnalysis(
          previewData,
          detectedColumns
        );
  ...
  private generateRecommendedMappings(
    columns: DetectedColumn[]
  ): ColumnMapping[] {
  ...
  private async performDataQualityAnalysis(
    ...
    columns.forEach((column) => {
      const columnValues = previewData
      ...
      const completenessScore =
        previewData.length > 0
          ? ((previewData.length - nullCount) / previewData.length) * 100
          : 0;
      ...
      const uniquenessScore =
        columnValues.length > 0
          ? (uniqueValues.size / columnValues.length) * 100
          : 0;
      ...
      const validityScore =
        columnValues.length > 0 ? (validCount / columnValues.length) * 100 : 0;
      ...
      if (column.type === "string" && columnValues.length > 1) {
        const formats = columnValues.map((val) =>
          this.detectValueFormat(String(val))
        );
      ...
      if (completenessScore < 80) {
        issues.push({
          ...
          columnName: column.name,
          message: `Low completeness: ${completenessScore.toFixed(
            1
      ...
      if (uniquenessScore < 50 && column.name.toLowerCase().includes("id")) {
        issues.push({
          ...
          category: "data_quality",
          columnName: column.name,
          message: `Potential duplicate IDs detected: ${uniquenessScore.toFixed(
            1
          )}% uniqueness`,
      ...
      if (validityScore < 90) {
        issues.push({
          ...
          category: "data_quality",
          columnName: column.name,
          message: `Data type validation failed: ${validityScore.toFixed(
            1
          )}% valid values`,
    ...
    const overallScore =
      ...
      (completeness.length * 4);

    // Generate recommendations
    const recommendations: string[] = [];
    if (overallScore < 70) {
      recommendations.push(
        "Data quality is below acceptable threshold. Consider data cleansing."
      );
    }
    ...
    if (issues.some((i) => i.category === "data_quality")) {
      recommendations.push(
        "Review data quality issues and consider preprocessing the data."
      );
    }
    ...
    const report: DataQualityReport = {
      ...
      issues: issues.map((issue) => ({
        type:
          issue.category === "data_quality" ? "missing_data" : "invalid_format",
  ...
  async executeImportJob(
    ...
    try {
      ...
      try {
        ...
        if (
          operationIndicator.cancellationToken?.token.isCancellationRequested
        ) {
        ...
        statusBarProvider.updateOperationStep(
          `import-${jobId}`,
          0,
          "completed"
        );
        ...
        const result = await this.performImport(
          job,
          columnMapping,
          operationIndicator.cancellationToken?.token
        );
  ...
  private async validateImportData(
    ...
    try {
      ...
      switch (job.format) {
        ...
        default:
          throw new Error(
            `Validation not implemented for format: ${job.format}`
          );
      }
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i += batchSize) {
        ...
        try {
          ...
          const transformedBatch = this.transformBatchData(
            batch,
            columnMapping
          );
          ...
          // Execute batch via native service - use PostgreSqlConnectionManager
          const result = await this.dotNetService.createConnection(dotNetConnection).then(async (handle) => {
            try {
              const queryResult = await handle.connection.query(sql);
              return { rowCount: queryResult.rowCount, executionTime: 0 };
            } finally {
              handle.release();
            }
          });
            ...
            <body>
                ...
                ${job.errors
                  .map(
                    (error) => `
                    <div class="error">
                        <strong>Row ${error.rowNumber}${
                      error.columnName ? `, Column ${error.columnName}` : ""
                    }:</strong><br>
                        ${error.message}<br>
                        <small>Type: ${error.errorType} | Severity: ${
                      error.severity
                    }</small>
                    </div>
                `
  ...
  getImportStatistics(): {
    ...
    const totalRows = completedJobs.reduce(
      (sum, job) => sum + (job.importedRows || 0),
      0
    );
