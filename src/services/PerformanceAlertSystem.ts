...
import { PerformanceAlert, PerformanceMonitorService } from "@/services/PerformanceMonitorService";
...
export class PerformanceAlertSystem {
	...
	getAlertRules(): AlertRule[] {
		return Array.from(this.alertRules.values()).sort((a, b) => b.updatedAt.getTime() - a.updatedAt.getTime());
	}
...
class AlertTreeProvider implements vscode.TreeDataProvider<AlertTreeItem> {
	private _onDidChangeTreeData: vscode.EventEmitter<AlertTreeItem | undefined | null> = new vscode.EventEmitter<
		AlertTreeItem | undefined | null
	>();
	...
	getChildren(element?: AlertTreeItem): Thenable<AlertTreeItem[]> {
		if (!element) {
			...
			return Promise.resolve([
				new AlertTreeItem("Active Alerts", vscode.TreeItemCollapsibleState.Expanded, "activeAlerts", "alerts"),
				new AlertTreeItem("Alert Rules", vscode.TreeItemCollapsibleState.Collapsed, "alertRules", "settings"),
				new AlertTreeItem("Statistics", vscode.TreeItemCollapsibleState.Collapsed, "statistics", "graph"),
			]);
		}
