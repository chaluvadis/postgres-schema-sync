...
public class SchemaMetadataExtractor(ILogger<SchemaMetadataExtractor> logger) : ISchemaMetadataExtractor
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            _logger.LogInformation("Starting parallel extraction of all database objects");

            // Extract different object types in parallel for better performance
            var tablesTask = ExtractTablesAsync(connection, schemaFilter, cancellationToken);
            var viewsTask = ExtractViewsAsync(connection, schemaFilter, cancellationToken);
            var functionsTask = ExtractFunctionsAsync(connection, schemaFilter, cancellationToken);
            var proceduresTask = ExtractProceduresAsync(connection, schemaFilter, cancellationToken);
            var sequencesTask = ExtractSequencesAsync(connection, schemaFilter, cancellationToken);
            var typesTask = ExtractTypesAsync(connection, schemaFilter, cancellationToken);
            var indexesTask = ExtractIndexesAsync(connection, schemaFilter, cancellationToken);
            var triggersTask = ExtractTriggersAsync(connection, schemaFilter, cancellationToken);
            var domainsTask = ExtractDomainsAsync(connection, schemaFilter, cancellationToken);
            var collationsTask = ExtractCollationsAsync(connection, schemaFilter, cancellationToken);
            var extensionsTask = ExtractExtensionsAsync(connection, schemaFilter, cancellationToken);
            var rolesTask = ExtractRolesAsync(connection, schemaFilter, cancellationToken);
            var tablespacesTask = ExtractTablespacesAsync(connection, schemaFilter, cancellationToken);

            await Task.WhenAll(tablesTask, viewsTask, functionsTask, proceduresTask,
                             sequencesTask, typesTask, indexesTask, triggersTask,
                             domainsTask, collationsTask, extensionsTask, rolesTask, tablespacesTask);

            var allObjects = new List<DatabaseObject>();
            allObjects.AddRange(await tablesTask);
            allObjects.AddRange(await viewsTask);
            allObjects.AddRange(await functionsTask);
            allObjects.AddRange(await proceduresTask);
            allObjects.AddRange(await sequencesTask);
            allObjects.AddRange(await typesTask);
            allObjects.AddRange(await indexesTask);
            allObjects.AddRange(await triggersTask);
            allObjects.AddRange(await domainsTask);
            allObjects.AddRange(await collationsTask);
            allObjects.AddRange(await extensionsTask);
            allObjects.AddRange(await rolesTask);
            allObjects.AddRange(await tablespacesTask);

            _logger.LogInformation("Successfully extracted {ObjectCount} database objects from {Database}",
                allObjects.Count, connection.Database);
            return allObjects;
        }
        catch (Exception ex)
        {
    ...
    public async Task<List<Function>> ExtractProceduresAsync(
        NpgsqlConnection connection,
        string? schemaFilter = null,
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var query = @"
                ...
                JOIN pg_language l ON p.prolang = l.oid
                WHERE n.nspname NOT IN ('information_schema', 'pg_catalog')
                AND p.prokind = 'p'  -- 'p' for procedures, 'f' for functions
                AND (@SchemaFilter IS NULL OR n.nspname = @SchemaFilter)
                GROUP BY p.proname, n.nspname, p.oid, l.lanname, p.provolatile, p.prosecdef, p.proparallel, p.proowner
                ORDER BY n.nspname, p.proname";
            ...
            while (await reader.ReadAsync(cancellationToken))
            {
                var procedure = new Function
                {
                    Name = reader.GetString(0),
                    Schema = reader.GetString(1),
                    ReturnType = reader.GetString(3), // Always 'void' for procedures
                    Language = reader.GetString(5),
                    Type = ObjectType.Procedure, // Use Procedure type if available, otherwise Function
                    Database = connection.Database,
                    Owner = reader.GetString(10),
                    Volatility = reader.GetString(6),
                    Properties = new Dictionary<string, string>
                ...
                // Extract procedure body if needed
                procedure.SourceCode = await GetProcedureSourceAsync(connection, procedure.Schema, procedure.Name, cancellationToken);

                procedures.Add(procedure);
            }

            _logger.LogInformation("Extracted {ProcedureCount} procedures from {Database}",
                procedures.Count, connection.Database);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to extract procedures");
            throw;
        }
    ...
    public async Task<List<Domain>> ExtractDomainsAsync(
        NpgsqlConnection connection,
        string? schemaFilter = null,
        CancellationToken cancellationToken = default)
    {
        ...
        var query = @"
            SELECT
                ...
                pg_get_domain_type(t.oid) as base_type,
                t.typdefault as default_value,
                pg_get_constraintdef(con.oid) as check_constraint,
                t.typowner::regrole::text as owner
            FROM pg_type t
            JOIN pg_namespace n ON t.typnamespace = n.oid
            LEFT JOIN pg_constraint con ON t.oid = con.contypid AND con.contype = 'c'
            WHERE t.typtype = 'd'
            AND n.nspname NOT IN ('information_schema', 'pg_catalog')
            AND (@SchemaFilter IS NULL OR n.nspname = @SchemaFilter)
            ORDER BY n.nspname, t.typname";
        ...
        while (await reader.ReadAsync(cancellationToken))
        {
            ...
            if (!reader.IsDBNull(3))
                domain.DefaultValue = reader.GetString(3);
            if (!reader.IsDBNull(4))
                domain.CheckConstraint = reader.GetString(4);
    ...
    public async Task<List<Collation>> ExtractCollationsAsync(
        NpgsqlConnection connection,
        string? schemaFilter = null,
        CancellationToken cancellationToken = default)
    {
        var collations = new List<Collation>();
        var query = @"
            SELECT
                c.collname as collation_name,
                n.nspname as collation_schema,
                c.collprovider as provider,
                c.colldeterministic as is_deterministic,
        ...
        while (await reader.ReadAsync(cancellationToken))
        {
            var collation = new Collation
            {
                ...
                IsDeterministic = reader.GetBoolean(3),
            ...
            if (!reader.IsDBNull(4))
                collation.Collate = reader.GetString(4);
            if (!reader.IsDBNull(5))
                collation.Ctype = reader.GetString(5);
    ...
    public async Task<List<Extension>> ExtractExtensionsAsync(
        NpgsqlConnection connection,
        string? schemaFilter = null,
        CancellationToken cancellationToken = default)
    {
        ...
        while (await reader.ReadAsync(cancellationToken))
        {
            var extension = new Extension
            {
                ...
                Properties = new Dictionary<string, string>
                {
                    ["IsRelocatable"] = reader.GetBoolean(3).ToString()
                }
            };
    ...
    public async Task<List<Role>> ExtractRolesAsync(
        NpgsqlConnection connection,
        string? schemaFilter = null,
        CancellationToken cancellationToken = default)
    {
        ...
        var query = @"
            SELECT
                ...
                array_agg(rm.rolname) FILTER (WHERE rm.rolname IS NOT NULL) as member_of
            FROM pg_roles r
            LEFT JOIN pg_auth_members am ON r.oid = am.member
            LEFT JOIN pg_roles rm ON am.roleid = rm.oid
            GROUP BY r.oid, r.rolname, r.rolsuper, r.rolcreatedb, r.rolcreaterole, r.rolcanlogin, r.rolvaliduntil
            ORDER BY r.rolname";
        ...
        while (await reader.ReadAsync(cancellationToken))
        {
            var role = new Role
            {
                ...
                CanCreateDatabases = reader.GetBoolean(2),
            ...
            if (!reader.IsDBNull(5))
                role.ValidUntil = reader.GetString(5);
            if (!reader.IsDBNull(6))
                role.MemberOf = reader.GetFieldValue<string[]>(6).ToList();
    ...
    public async Task<List<Tablespace>> ExtractTablespacesAsync(
        NpgsqlConnection connection,
        string? schemaFilter = null,
        CancellationToken cancellationToken = default)
    {
        var tablespaces = new List<Tablespace>();
        var query = @"
            SELECT
                t.spcname as tablespace_name,
                t.spcowner::regrole::text as owner,
        ...
        while (await reader.ReadAsync(cancellationToken))
        {
            ...
            if (!reader.IsDBNull(4))
            {
                ...
                var options = reader.GetFieldValue<string[]>(4);
                foreach (var option in options)
                {
                    var parts = option.Split('=');
                    if (parts.Length == 2)
                    {
                        tablespace.Options[parts[0]] = parts[1];
                    }
                }
            }
    ...
    private async Task<string> GetProcedureSourceAsync(
        NpgsqlConnection connection,
        string schema,
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            return result?.ToString() ?? $"-- Could not retrieve source for procedure {schema}.{procedureName}";
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Failed to get source for procedure {ProcedureName}", procedureName);
            return $"-- Could not retrieve source for procedure {schema}.{procedureName}: {ex.Message}";
        }
    }
}